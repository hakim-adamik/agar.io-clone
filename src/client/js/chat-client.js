...
class ChatClient {
    ...
    registerFunctions() {
        ...
        this.registerCommand(
            "fps",
            "Toggle FPS/UPS counter display.",
            function () {
                self.toggleFpsDisplay();
            }
        );
    ...
    addChatLine(name, message, me) {
        ...
        newline.innerHTML =
            "<b>" +
            (name.length < 1 ? "An unnamed cell" : name) +
            "</b>: " +
            message;
    ...
    toggleFpsDisplay() {
        if (!global.showFpsCounter) {
            global.showFpsCounter = true;
            if (global.fpsCounter) {
                global.fpsCounter.style.display = "block";
            }
            this.addSystemLine("FPS/UPS counter enabled.");
        } else {
            global.showFpsCounter = false;
            if (global.fpsCounter) {
                global.fpsCounter.style.display = "none";
            }
            this.addSystemLine("FPS/UPS counter disabled.");
        }
        // Update checkbox to match state
        ...
        try {
            localStorage.setItem("showFpsCounter", global.showFpsCounter);
        } catch (e) {
